...
class SimpleTestCases(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_user = {
            'username': 'arthur', 'email': 'arthur@mail.com', 'password': '1278923'
        }
        self.test_business = {
            'username': 'Andrew', 'business_name': 'Shopping',
            'location': 'kabete', 'description': 'Self-service shop'
        }
        self.test_business_name = 'a_business'
        self.test_business_reviews = {'a_business': 'rattotouille', 'fencefaced':'Clubbard'}
    ...
    def test_register_http_response_status(self):
        response = self.app.post(
            '/v1/auth/register', data=json.dumps(self.test_user),
    ...
    def test_login_http_response_status(self):
        response = self.app.post(
            '/v1/auth/login', data=json.dumps(self.test_user),
            headers={'content-type': 'application/json'}
        )
        self.assertEqual(201, response.status_code)
    ...
    def test_logout_returns_approp_msg(self):
        ...
        response = self.app.post(
            "/v1/auth/logout", data=json.dumps(logout_msg),
            headers={'content-type': 'application/json'}
        )
    ...
    def test_password_reset_response_status(self):
        response = self.app.post(
            '/v1/auth/reset-password', data=json.dumps(self.test_user),
            headers={'content-type': 'application/json'}
        )
        self.assertEqual(201, response.status_code)
